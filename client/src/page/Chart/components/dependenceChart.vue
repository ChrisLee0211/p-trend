...
export default defineComponent({
    ...
    setup() {
        ...
        const getFileDepsUrl = computed(() => `http://localhost:${window.preloadState.port}/chart/deps?limit=${limit.value}`)
        const { onFetchResponse, data, onFetchError } = useFetch<{nodes:FileNode[]}>(getFileDepsUrl, { refetch:true }).json<{nodes:FileNode[]}>();
        const chartData = computed(() => {
            if(data.value && data.value.nodes) {
                return data.value.nodes.map((node) => ({...node, depCount:node.deps.length})).sort((a,b) => b.depCount - a.depCount);
            }
            return [];
        })
        onFetchError((param) => {
